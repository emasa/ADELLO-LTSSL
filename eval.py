...
if __name__ == "__main__":
    ...
    parser.add_argument('--out-key', type=str, default='logits')
    ...
    args.num_labels = 4 * args.num_classes
    ...
    with torch.no_grad():
        for data in eval_loader:
            ...
            out_dict = net(image)
            logit, feat = out_dict[args.out_key], out_dict['feat']
    ...
    # calibration error
    test_probs, test_labels = torch.from_numpy(test_probs), torch.from_numpy(test_labels)
    ece = multiclass_calibration_error(test_probs, test_labels, num_classes=args.num_classes, norm='l1').item()
    mce = multiclass_calibration_error(test_probs, test_labels, num_classes=args.num_classes, norm='max').item()

    print(f"Test Accuracy: {100 * acc:.2f} ; ECE: {100 * ece:.2f} ; MCE: {100 * mce:.2f}")
