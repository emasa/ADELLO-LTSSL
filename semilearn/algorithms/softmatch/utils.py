...
class SoftMatchWeightingHook(MaskingHook):
    ...
    """
    def __init__(self, num_classes, n_sigma=2, momentum=0.999, per_class=False, init_mu=None, init_var=None, *args, **kwargs):
        ...
        init_mu = (1.0 / self.num_classes) if (init_mu is None) else init_mu
        init_var = 1.0 if (init_var is None) else init_var

        if not self.per_class:
            self.prob_max_mu_t = torch.tensor(init_mu)
            self.prob_max_var_t = torch.tensor(init_var)
        else:
            self.prob_max_mu_t = torch.ones((self.num_classes)) * init_mu
            self.prob_max_var_t = torch.ones((self.num_classes)) * init_var
