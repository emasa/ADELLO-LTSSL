...
class CRMatch(AlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        self.init(p_cutoff=args.p_cutoff, hard_label=args.hard_label, lambda_cos=self.args.lambda_cos)

    def init(self, p_cutoff, hard_label=True, lambda_cos=1.0):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s, x_ulb_rot=None, rot_v=None):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                y_ulb = torch.argmax(probs_x_ulb_w, dim=-1)
                mask = self.call_hook("masking", "MaskingHook", logits_x_ulb=probs_x_ulb_w, softmax_x_ulb=False)

            Lx = self.ce_loss(logits_x_lb, y_lb, reduction='mean')
            Lu = self.consistency_loss(logits_x_ulb_s, y_ulb, 'ce', mask=mask)
            ...
            total_loss = Lx + self.lambda_u * Lu + self.lambda_cos * Ld
            ...
            if self.use_rot:
                ...
                total_loss += self.lambda_rot * Lrot

            if self.args.distill_complement:
                distill_mask = 1 - mask
                distill_loss, distill_temperature, _ = self.compute_distill_loss(logits_x_ulb_s, probs_x_ulb_w, distill_mask)
                total_loss += self.args.lambda_distill * distill_loss
        ...
        if self.use_rot:
            log_dict.update(dict(rot_loss=Lrot.item()))
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--lambda_cos', float, 1.0, 'weight for cosine loss'),
            SSL_Argument('--p_cutoff', float, 0.95),

            SSL_Argument('--lambda_distill', float, 1.0),
            SSL_Argument('--distill_complement', str2bool, False),
            SSL_Argument('--distill_T', float, None),
        ]
