...
from semilearn.algorithms.utils import SSL_Argument, concat_all_gather, str2bool
...
class SimMatch(AlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        self.init(T=args.T, p_cutoff=args.p_cutoff, proj_size=args.proj_size, K=args.K, smoothing_alpha=args.smoothing_alpha,
                  da_len=args.da_len, dist_align=args.dist_align)
    

    def init(self, T, p_cutoff, proj_size, K, smoothing_alpha, da_len=0, dist_align=True):
        self.T = T 
    ...
    def train_step(self, idx_lb, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                # ema_probs_x_ulb_w = torch.softmax(ema_logits_x_ulb_w.detach(), dim=1)
                ema_probs_x_ulb_w = self.compute_prob(ema_logits_x_ulb_w.detach())
                if self.dist_align:
                    ema_probs_x_ulb_w = self.call_hook("dist_align", "DistAlignHook", probs_x_ulb=ema_probs_x_ulb_w)
