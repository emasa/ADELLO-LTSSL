...
class CoMatch(AlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        self.init(T=args.T, p_cutoff=args.p_cutoff, 
                  ...
                  queue_batch=args.queue_batch, smoothing_alpha=args.smoothing_alpha, da_len=args.da_len, dist_align=args.dist_align)
        self.lambda_c = args.contrast_loss_ratio

    def init(self, T, p_cutoff, contrast_p_cutoff, hard_label=True, queue_batch=128, smoothing_alpha=0.999, da_len=256, dist_align=True):
        self.T = T 
    ...
    def set_hooks(self):
        self.register_hook(
            DistAlignQueueHook(num_classes=self.num_classes, queue_length=self.args.da_len, p_target_type='uniform',
                               smooth_p_target=self.args.smooth_p_target, smooth_p_model=self.args.smooth_p_model),
            "DistAlignHook")
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s_0, x_ulb_s_1):
        ...
        with self.amp_cm():
            ...
            with torch.no_grad():
                ...
                # probs = torch.softmax(logits_x_ulb_w.detach(), dim=1)
                probs = self.compute_prob(logits_x_ulb_w.detach())
                # distribution alignment
                if self.dist_align:
                    probs = self.call_hook("dist_align", "DistAlignHook", probs_x_ulb=probs)
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--smooth_p_target', str2bool, False),
            SSL_Argument('--smooth_p_model', str2bool, False),
        ]
