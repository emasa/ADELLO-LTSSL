...
class ReMixMatch(AlgorithmBase):
    ...
    def set_hooks(self):
        if self.args.dist_align_target == 'uniform':
            lb_class_dist = [1 for _ in range(self.num_classes)]
        else:  # labeled prior
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s_0, x_ulb_s_1, x_ulb_s_0_rot=None, rot_v=None):
        ...
        with self.amp_cm():
            with torch.no_grad():
                ...
                prob_x_ulb = self.call_hook("dist_align", "DistAlignHook", probs_x_ulb=self.compute_prob(logits_x_ulb_w.detach()))
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--dist_align_target', str, 'labeled', 'DA target distribution'),
        ]
