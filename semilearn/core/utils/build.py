...
from semilearn.datasets import get_collactor, name2sampler, get_small_imagenet
...
def get_net_builder(net_name, from_name: bool):
    ...
    if from_name:
        ...
        if net_name == 'abc_net':
            from ...imb_algorithms.abc import abc_net
            builder = abc_net
        elif net_name == 'cossl_net':
            from ...imb_algorithms.cossl import cossl_net
            builder = cossl_net
        else:
...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True,
                labeled_transform='weak', return_lb_in_ulb_mask=False):
    """
    create dataset
    ...
    if dataset == "eurosat":
        ...
        lb_dset, ulb_dset, eval_dset = get_medmnist(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir,  include_lb_to_ulb=include_lb_to_ulb,
                                                    return_lb_in_ulb_mask=return_lb_in_ulb_mask)
        test_dset = None
    elif dataset == "semi_aves":
        lb_dset, ulb_dset, eval_dset = get_semi_aves(args, algorithm, dataset, train_split='l_train_val', ulb_split='u_train_in', data_dir=data_dir)
        test_dset = None
        ...
        test_dset = None
    elif dataset == "semi_aves_in_all":
        lb_dset, ulb_dset, eval_dset = get_semi_aves(args, algorithm, "semi_aves", train_split='l_train_val', ulb_split='u_train_in_all', data_dir=data_dir)
        test_dset = None
    elif dataset == "semi_aves_all":
        lb_dset, ulb_dset, eval_dset = get_semi_aves(args, algorithm, "semi_aves", train_split='l_train_val', ulb_split='u_train_all', data_dir=data_dir)
        test_dset = None
        ...
        lb_dset, ulb_dset, eval_dset = get_cifar(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb,
                                                 labeled_transform=labeled_transform, return_lb_in_ulb_mask=return_lb_in_ulb_mask)
        test_dset = None
    elif dataset == 'svhn':
        ...
        test_dset = None
    elif dataset in ["small_imagenet", "small_imagenet127"]:
        lb_dset, ulb_dset, eval_dset = get_small_imagenet(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
...
def get_optimizer(net, optim_name='SGD', lr=0.1, momentum=0.9, weight_decay=0, layer_decay=1.0, nesterov=True, bn_wd_skip=True):
    ...
    Args:
        ...
        optim_name: optimizer name, SGD|AdamW|Adam
        lr: learning rate
        momentum: momentum parameter for SGD
        weight_decay: weight decay in optimizer (used in SGD|AdamW)
...
def get_multistep_schedule(optimizer, milestones, gamma=0.1):
