...
class EMAN:
    """
    Improved EMA model (includes warmup and update of BN buffers)
    Original EMA implementation from https://fyubang.com/2019/06/01/ema/
    """
    ...
    def load(self, ema_model):
        ...
        for name, param in ema_model.named_buffers():
            self.shadow[name] = param.data.clone()
    ...
    def update(self, step=None):
        if step is not None:
            ema_decay = min(1 - 1 / (step + 1), self.decay)  # EMA warmup
        else:
