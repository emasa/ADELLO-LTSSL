...
from semilearn.core.hooks import Hook, get_priority, CheckpointHook, TimerHook, LoggingHook, DistSamplerSeedHook, ParamUpdateHook, EvaluationHook, EMAHook, EMANHook, WANDBHook, AimHook
from semilearn.core.utils import get_dataset, get_data_loader, get_optimizer, get_cosine_schedule_with_warmup, \
    Bn_Controller, get_multistep_schedule
...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        # set buffers to store metrics values
        self._test_top1_accs = []
        self._test_balanced_accs = []
    ...
    def set_dataset(self):
        ...
        dataset_dict = get_dataset(self.args, self.algorithm, self.args.dataset, self.args.num_labels, self.args.num_classes,
                                   self.args.data_dir, self.args.include_lb_to_ulb, self.args.labeled_transform,
                                   return_lb_in_ulb_mask=self.args.return_lb_in_ulb_mask)
    ...
    def set_hooks(self):
        ...
        if self.args.use_improved_ema:
            self.register_hook(EMANHook(), None, "HIGH")
        else:
    ...
    def compute_prob(self, logits, **kwargs):
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False, track_mean_acc=True):
        """
        evaluation function
        """
        self.model.eval()
        ...
        y_probs, y_true = torch.from_numpy(y_logits).softmax(dim=-1), torch.from_numpy(y_true)
        ece = multiclass_calibration_error(y_probs, y_true, num_classes=self.num_classes, norm='l1').item()
        mce = multiclass_calibration_error(y_probs, y_true, num_classes=self.num_classes, norm='max').item()

        self.ema.restore()
        self.model.train()

        with np.printoptions(threshold=np.inf):
            self.print_fn(eval_dest+'/confusion matrix:\n' + np.array_str(cf_mat))

        eval_dict = {
            eval_dest+'/loss': total_loss / total_num,
            eval_dest+'/top-1-acc': top1, eval_dest+'/balanced_acc': balanced_top1,
            eval_dest+'/precision': precision, eval_dest+'/recall': recall, eval_dest+'/F1': F1,
            eval_dest+'/ece': ece, eval_dest+'/mce': mce,
        }

        if track_mean_acc:
            self._test_top1_accs.append(top1)
            self._test_balanced_accs.append(balanced_top1)
            eval_dict.update({
                     eval_dest+'/top-1-acc-mean-last-20': np.mean(self._test_top1_accs[-20:]),
                     eval_dest+'/balanced-acc-mean-last-20': np.mean(self._test_balanced_accs[-20:]),
                     eval_dest+'/top-1-acc-median-last-20': np.median(self._test_top1_accs[-20:]),
                     eval_dest+'/balanced-acc-median-last-20': np.median(self._test_balanced_accs[-20:]),
            })
    ...
    def set_optimizer(self):
        if 'vit' in self.args.net and self.args.dataset in ['cifar100', 'food101', 'semi_aves', 'semi_aves_out']:
            ...
            if self.args.use_scheduler:
                assert self.args.scheduler_type in ('cosine', 'multistep')
                ...
                if self.args.scheduler_type == 'cosine':
                    ...
                    scheduler = get_multistep_schedule(optimizer,
                                                       milestones=self.args.multistep_milestones,
                                                       gamma=self.args.multistep_gamma)
            else:
                scheduler = None
