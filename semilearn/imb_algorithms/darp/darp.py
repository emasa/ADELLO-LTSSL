...
class DARP(ImbAlgorithmBase):
    ...
    def set_hooks(self):
        ...
        if self.args.darp_precomputed_u_prior_file is not None:
            self.print_fn('Loading precomputed unlabeled prior from: {}'.format(self.args.darp_precomputed_u_prior_file))
            target_disb = np.load(self.args.darp_precomputed_u_prior_file)
            self.print_fn(f'Precomputed prior: {target_disb}')
        else:
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--darp_precomputed_u_prior_file', str, None),
        ]
