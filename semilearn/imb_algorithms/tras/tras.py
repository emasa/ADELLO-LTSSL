...
class TRAS(ImbAlgorithmBase):
    def __init__(self, args, **kwargs):
        ...
        assert args.algorithm == 'fixmatch', "TRAS only supports FixMatch as the base algorithm."
    ...
    def train_step(self, *args, **kwargs):
        ...
        # get teacher logits
        logits_x_ulb_teacher = out_dict['logits']['x_ulb_s']
        
        # get logits for auxiliary classifier
        ...
        tras_unsup_loss = self.tras_kl_loss(logits_x_ulb_s, logits_x_ulb_teacher.detach()-la_u, mask)
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False, **kwargs):
        if self.epoch < self.warmup_epochs:
            ...
            out_key = 'logits_aux'
        return super().evaluate(eval_dest=eval_dest, out_key=out_key, return_logits=return_logits, **kwargs)
