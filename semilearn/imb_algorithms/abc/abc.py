...
class ABCNet(nn.Module):
    def __init__(self, backbone=None, num_classes=None):
        super().__init__()
        ...
        # auxilary classifier
        self.aux_classifier = nn.Linear(self.num_features, num_classes)
...
class ABC(ImbAlgorithmBase):
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False, **kwargs):
        return super().evaluate(eval_dest=eval_dest, out_key='logits_aux', return_logits=return_logits, **kwargs)
...
def abc_net(pretrained=False, pretrained_path=None, net_name='wrn_28_2', num_classes=10, **kwargs):
    backbone = getattr(nets, net_name)(num_classes=num_classes, pretrained=False, **kwargs)
    model = ABCNet(backbone, num_classes)
    if pretrained:
