...
class ACR(ImbAlgorithmBase):
    ...
    def train_step(self, *args, **kwargs):
        ...
        with self.amp_cm():
            ...
            # ACR trick for FixMatch
            # get mask of originally labeled samples within unlabeled batch
            lb_in_ulb_mask = kwargs.get('lb_in_ulb_mask', None)
            if lb_in_ulb_mask is None:
                mask_l = torch.ones_like(mask_b)  # no mask available
            else:
                mask_l = lb_in_ulb_mask < 1  # mask out labeled data
            mask_l, mask_b = mask_l.float(), mask_b.float()
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False, **kwargs):
        return super().evaluate(eval_dest=eval_dest, out_key='logits_aux', return_logits=False, **kwargs)
