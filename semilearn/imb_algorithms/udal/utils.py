...
class UDALHook(Hook):
    ...
    def before_train_step(self, algorithm):
        algorithm.consistency_loss.set_params(p_hat=algorithm.get_p_hat(), cte_iter=algorithm.it,
                                              max_iter=algorithm.num_train_iter,
                                              num_iter_per_epoch=algorithm.num_iter_per_epoch)
        algorithm.ce_loss.set_params(p_hat=algorithm.get_p_hat(), cte_iter=algorithm.it,
                                     max_iter=algorithm.num_train_iter,
                                     num_iter_per_epoch=algorithm.num_iter_per_epoch)
    ...
    def _track_unl_prior_estimation(self, algorithm):
        ...
        if p_hat is not None:
            fwd_kl_div_p_data, _, _ = compute_divergences(p_hat, p_data)
...
def compute_udal_adjustment_dist(current_dist, p_target, current_epoch, max_epoch, a_min=0.1, k=2.0, a_max=1.0, eps=1e-9):
    ...
    # Note: UDAL didn't include eps on numerator originally
    return (current_dist + eps) / (target_dist + eps)


class UDALSupervisedLoss(CELoss):
    def __init__(self, alpha_min=0.1, k=2.0, p_data=None, p_hat=None, p_target=None, target_mode='udal', use_epochs=True,
    ...
    def forward(self, logits, targets, reduction='mean', T_src=None):
        assert self.target_mode in ['udal', 'udal_dynamic', 'udal_gt']
    ...
    def get_target_dist(self):
        assert self.target_mode in ['udal', 'udal_gt', 'udal_dynamic']
        return self.p_data


class UDALConsistencyLoss(ConsistencyLoss):
    def __init__(self, alpha_min=0.1, k=2.0, p_data=None, p_hat=None, p_target=None, target_mode='our_udal', use_epochs=True,
                 num_samples_lb=None, num_samples_ulb=None):
        super().__init__()
        self.progressive_alpha_min = alpha_min
        self.progressive_k = k
    ...
    def get_target_dist(self):
        ...
        if self.target_mode in ['udal', 'udal_gt']:
            p_target = self.p_target
        elif self.target_mode in ['udal_dynamic']:
            p_target = self.p_hat
        else:
            p_target = None
