...
class MedMNIST3D(MedMNIST):
    ...
    def __getitem__(self, index):
        ...
        if self.lb_in_ulb_mask is None:
            ...
            return img, target, self.lb_in_ulb_mask[index]
...
def get_medmnist(args, alg, dataset_name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, return_lb_in_ulb_mask=False):
    ...
    assert len(train_targets) == len(train_data), "MedMNIST dataset has an error!!!"
    ...
    train_labeled_data, train_labeled_targets, train_unlabeled_data, train_unlabeled_targets, lb_in_ulb_mask = split_ssl_data(args, total_data, total_targets, num_classes,
                                                                                                                              lb_num_labels=num_labels,
                                                                                                                              ulb_num_labels=args.ulb_num_labels,
                                                                                                                              lb_imbalance_ratio=args.lb_imb_ratio,
                                                                                                                              ulb_imbalance_ratio=args.ulb_imb_ratio,
                                                                                                                              include_lb_to_ulb=include_lb_to_ulb,
                                                                                                                              return_lb_in_ulb_mask=True)
    ...
    train_unlabeled_dataset = name2class[dataset_name](alg, root=data_dir, split="train", is_ulb=True, transform=transform_weak, transform_strong=transform_strong, as_rgb=True,
                                                       lb_in_ulb_mask=lb_in_ulb_mask if return_lb_in_ulb_mask else None)
