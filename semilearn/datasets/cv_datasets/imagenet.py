...
def get_imagenet(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, return_lb_in_ulb_mask=False):
    ...
    ulb_dset = ImagenetDataset(root=os.path.join(data_dir, "train"), transform=transform_weak, alg=alg, ulb=True,
                               strong_transform=transform_strong, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_dset.lb_idx,
...
class ImagenetDataset(BasicDataset, ImageFolder):
    def __init__(self, root, transform, ulb, alg, strong_transform=None, percentage=-1, include_lb_to_ulb=True, lb_index=None,
        ...
        samples = self.make_dataset(self.root, class_to_idx, extensions, is_valid_file, return_lb_in_ulb_mask)
        if len(samples) == 0:
        ...
        self.lb_in_ulb_mask = [s[2] for s in samples] if return_lb_in_ulb_mask else None
    ...
    def __sample__(self, index):
        ...
        if self.lb_in_ulb_mask is None:
            ...
            return sample, target, self.lb_in_ulb_mask[index]
            ...
            for root, _, fnames in sorted(os.walk(target_dir, followlinks=True)):
                ...
                for fname in fnames:
                    ...
                    if is_valid_file(path):
                        if return_lb_in_ulb_mask:
                            lb_in_ulb_mask = 1 if (fname in self.lb_index[target_class]) else 0
                            item = path, class_index, lb_in_ulb_mask
                        else:
