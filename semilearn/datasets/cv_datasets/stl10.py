...
def get_stl10(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=False, return_lb_in_ulb_mask=False):
    ...
    lb_in_ulb_mask = np.zeros_like(ulb_targets)
    if include_lb_to_ulb:
        lb_in_ulb_mask = np.concatenate([np.ones_like(lb_targets), lb_in_ulb_mask], axis=0)
