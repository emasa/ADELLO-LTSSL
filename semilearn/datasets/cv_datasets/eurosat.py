...
def get_eurosat(args, alg, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, return_lb_in_ulb_mask=False):
    ...
    # FIXME: add additional data folder to avoid indexing the 'labeled_idx' autogenerated folder
    data_dir = os.path.join(data_dir, dataset.lower(), "data")
    ...
    train_labeled_idxs, _, train_unlabeled_idxs, _, lb_in_ulb_mask = split_ssl_data(args, total_idxs, total_targets, num_classes,
...
class EuroSat(ImageFolder, BasicDataset):
    ...
    def __init__(self, alg, root, split, is_ulb=False, idx_list=None, transform=None, target_transform=None, transform_strong=None, lb_in_ulb_mask=None):
        ...
        # FIXED: Sub-sampled target data (used for GT prior estimation)
        self.targets = np.asarray(self.targets)[np.asarray(self.idx_list)]
