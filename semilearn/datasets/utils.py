...
def split_ssl_data(args, data, targets, num_classes,
                   ...
                   lb_index=None, ulb_index=None, include_lb_to_ulb=True, load_exist=True, return_lb_in_ulb_mask=False):
    """
    ...
    lb_in_ulb_mask = np.zeros_like(ulb_idx)

    if include_lb_to_ulb:
        ulb_idx = np.concatenate([lb_idx, ulb_idx], axis=0)
        lb_in_ulb_mask = np.concatenate([np.ones_like(lb_idx), lb_in_ulb_mask], axis=0)

    if return_lb_in_ulb_mask:
        return data[lb_idx], targets[lb_idx], data[ulb_idx], targets[ulb_idx], lb_in_ulb_mask
    else:
