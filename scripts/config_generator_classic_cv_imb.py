...
"""
import os
from config_generator_classic_cv import create_classific_config
...
def create_configuration(cfg, cfg_file):
    if 'imb_algorithm' in cfg:
        cfg['save_name'] = "{alg}_{imb_alg}_{dataset}_lb{num_lb}_{imb_lb}_ulb{num_ulb}_{imb_ulb}_{seed}".format(
        ...
        cfg['save_name'] = "{alg}_{dataset}_lb{num_lb}_{imb_lb}_ulb{num_ulb}_{imb_ulb}_{seed}".format(
            alg=cfg['algorithm'],
            dataset=cfg['dataset'],
            num_lb=cfg['num_labels'],
            imb_lb=cfg['lb_imb_ratio'],
...
def create_classic_cv_imb_config(alg, seed, dataset, net, num_classes, num_labels, img_size, port, lr, weight_decay, 
                                 imb_alg, lb_imb_ratio, ulb_imb_ratio, ulb_num_labels,
                                 amp=False, layer_decay=1.0, pretrain_name=None):
    
    # get core algorithm related configs
    if dataset in ['cifar10', 'cifar100', 'stl10']:
        cfg = create_classific_config(alg, seed, dataset, net, num_classes, num_labels, img_size, port, weight_decay)
        cfg['net'] = net
        cfg['optim'] = 'SGD'
        cfg['lr'] = lr
        ...
        cfg = create_usb_cv_config(alg, seed, dataset, net, num_classes, num_labels, img_size, 0.875, port, lr, weight_decay, layer_decay, pretrain_name, warmup=0, amp=False)
    ...
    if imb_alg == 'crest+':
        ...
        if dataset in ['imagenet127']:
            cfg['udal_alpha_min'] = 0.1
        else:
            cfg['udal_alpha_min'] = 0.55

        cfg['udal_k'] = 2.0
        cfg['udal_ema_p'] = 0.999
        cfg['udal_mode'] = 'our_udal'
    else:
        pass
...
def exp_classic_cv_imb(settings):
    ...
    imb_algs = [None, 'crest', 'crest+', 'darp', 'abc', 'daso', 'saw', 'adsh', 'cossl', 'debiaspl', 'simis', 'udal']
            ...
            for dataset in datasets:
                for seed in seeds:
                    if dataset == 'cifar10':
                        ...
                        pretrain_name = 'https://github.com/microsoft/Semi-supervised-learning/releases/download/v.0.0.0/vit_small_patch16_224_mlp_im_1k_224.pth'
                    ...
                    if dataset in ['cifar10', 'cifar100']:
                        ...
                        for setting_dict in setting_dicts:
                            ...
                            if alg == 'supervised':
                                ...
                                cfg = create_classic_cv_imb_config(alg, seed, dataset, net, num_classes, lb_num_labels, img_size, port, lr, weight_decay,
                                                                None, lb_imb_ratio, ulb_imb_ratio, ulb_num_labels, amp=False)
                                count += 1
                                ...
                                # prepare the configuration file
                                cfg = create_classic_cv_imb_config(alg, seed, dataset, net, num_classes, lb_num_labels, img_size, port, lr, weight_decay,
                                                                imb_alg, lb_imb_ratio, ulb_imb_ratio, ulb_num_labels, amp=False)
                                count += 1
                                create_configuration(cfg, config_file)
                        ...
                        if alg == 'supervised':
                            ...
                            # prepare the configuration file
                            cfg = create_classic_cv_imb_config(alg, seed, dataset, net, num_classes, lb_num_labels, img_size, port, lr, weight_decay,
                                                               None, lb_imb_ratio, ulb_imb_ratio, ulb_num_labels, amp=False, layer_decay=layer_decay, pretrain_name=pretrain_name)
                            count += 1
                            create_configuration(cfg, config_file)
                        else:
                            port = dist_port[count]
                            # prepare the configuration file
                            cfg = create_classic_cv_imb_config(alg, seed, dataset, net, num_classes, lb_num_labels, img_size, port, lr, weight_decay,
                                                            imb_alg, lb_imb_ratio, ulb_imb_ratio, ulb_num_labels, amp=False, layer_decay=layer_decay, pretrain_name=pretrain_name)
                            count += 1
                            create_configuration(cfg, config_file)
...
if __name__ == '__main__':
    ...
    settings = {
        'cifar10': [
            {'lb_num_labels': 1500, 'ulb_num_labels': 3000, 'lb_imb_ratio': 100, 'ulb_imb_ratio': 100},
            {'lb_num_labels': 500, 'ulb_num_labels': 4000, 'lb_imb_ratio': 100, 'ulb_imb_ratio': 100},
    
            {'lb_num_labels': 1500, 'ulb_num_labels': 3000, 'lb_imb_ratio': 150, 'ulb_imb_ratio': 150},
            {'lb_num_labels': 500, 'ulb_num_labels': 4000, 'lb_imb_ratio': 150, 'ulb_imb_ratio': 150},
        ...
        'cifar100': [
            {'lb_num_labels': 150, 'ulb_num_labels': 300, 'lb_imb_ratio': 10, 'ulb_imb_ratio': 10},
    
            {'lb_num_labels': 150, 'ulb_num_labels': 300, 'lb_imb_ratio': 15, 'ulb_imb_ratio': 15},
